...
筛选规则以工作地点上海为硬性标准，并严格依据简历中的支付 B 端产品运营、商户生命周期、实验增长、行业解决方案和销售赋能经历。页面先独立计算匹配度，再由分数确定档位，不继承旧分档：A+ 为 90-100、A- 为 70-89、B 为 45-69、C 为 0-44。A+ 只保留简历直接覆盖的支付运营、B 端产品 GTM、行业解决方案或商户生命周期岗位；工具、商务英语、行业背景和资历等缺口直接参与扣分或封顶，页面默认按最终匹配度降序排列。
...
面试提升页位于 `#interview`，会从每天更新的 `jobs.json` 动态读取当前 A+ 与 A- 岗位。页面保留 12 道简历核心题和 30 道加练题，其中 14 道直接由简历逐条拆解，覆盖 4000 万美元与 5.23 倍增长、高潜线索、新手 SOP、阶梯权益、RFM 规则迭代、销售工具、商户问题指标、行业方案标准化、SaaS 与银行协同、标杆首客、5 家分公司复制和渠道转化；其余 16 道保留经营分析、客户需求、项目交付、合作谈判和行为面试。每题都有事实边界、回答框架、追问和失分点，简历加练题另有针对性范文并优先展示。页面聚焦选题、回答、反馈和范文，回答提示、真实经历卡与录音按需折叠；首次检查只给出一个应保留点和一个优先改进点，修改后可保存复练版、比较两个版本并完成一道追问。最多七张“真实经历卡”可复用到不同题目；范文只重组简历中可核验事实，选择具体岗位时会提醒按 JD 改写。

文字回答、两版检查记录、目标岗位、经历卡、追问和录音都只保存在当前浏览器，不会上传。录音使用 IndexedDB 自动留档，每题保留最近 3 条，全站最多 30 条且总量不超过约 100 MB；达到上限时自动删除最旧录音，页面被系统中断时不保存不完整的新录音。清除站点数据、使用无痕模式或浏览器回收存储空间仍可能删除留档，因此它不是长期备份。关键词检查只是一种弱提示，不判断经历真伪、因果逻辑、回答质量或录用概率，也不应为了通过检查堆术语或编数字。核心题存放在 `interview-plan.json`，通用高频题及答题指南存放在 `interview-question-bank.json`，核心题参考表达存放在 `interview-sample-answers.json`；题库加载失败时核心训练仍可使用，刷新岗位数据不会覆盖这些内容。
