...
网页包含独立的九项技能提升页。它以 68 个 A 档岗位为样本，把原有 13 项重叠内容去重为 9 项技能：P0 合并通用底盘与支付、英语、国际市场核心专项，P1 保留电商经营、研究洞察和实验验证；点击提升计划后直接显示九项技能，也可从左侧目录进入单项训练。六项 P0 均有懒加载题库，共 42 章 668 题：业务英语为 10 个主题关卡、每关 50 题，并另设按常用程度排序的 500 词支付业务英语词汇表；数据分析与经营诊断为 8 章 48 题，其余四项各 6 章 30 题。所有题库只使用单选和排序等点击式交互，可跳过作答、直接查看参考答案或自由切换题目；词汇表支持分别遮盖中文释义、英文例句和中文翻译，并记录掌握状态。

关卡首页会从未作答的内容中随机生成“今日五题”，当天题组保持不变，同时记录每周练习天数；完成章节后会解锁一张可复用的速查卡。通用技能内容独立存放在 `learning-guide.json`，六套 P0 题库存放在 `challenges/` 并仅在进入对应技能时懒加载；业务英语进一步拆为轻量目录、10 个关卡文件和独立词汇数据，进入具体模块时才按需加载。各题库的学习进度、每日任务、词汇掌握状态和点击选择分别保存在当前浏览器。内容依据上级项目中的 `sources/recruit/A档岗位能力提升完整指南.md` 整理，因此刷新岗位数据时不会覆盖学习内容。
