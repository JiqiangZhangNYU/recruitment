...
网页包含独立的能力提升工作台。它以 68 个 A 档岗位为样本，首页介绍 13 项能力及其关系，并可从首页或侧栏进入单项学习路径、参考资料、习题和验收标准；另有 16 周执行路线、14 项作品集和 A-/A+ 投递检查清单。指南数据在首次进入提升计划时加载，单项详情、路线和作品视图按需渲染。所有学习进度只保存在当前浏览器。
