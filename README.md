...
面试提升页位于 `#interview`，会从每天更新的 `jobs.json` 动态读取当前 A+ 与 A- 岗位及其能力维度。首版包含 12 道题，覆盖自我介绍、代表项目、未达标项目复盘、商家增长、支付成功率、A/B 实验、跨境方案商业化、跨团队推进、英文项目复盘和 90 天计划。每题依次展示考察意图、回答框架和自由回答框；提交后由浏览器内的可解释规则检查个人贡献、指标证据、业务方法、取舍与复盘，并给出最多两项优先改进和一道追问。回答、目标岗位和诊断进度只保存在当前浏览器，不会上传；规则只检查结构和证据线索，不判断经历真伪或录用概率。题目和检查项独立存放在 `interview-plan.json`，刷新岗位数据不会覆盖。
