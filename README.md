...
线上页面通过 jsDelivr 从固定 Git 提交按需读取这些音频，读取失败时回退到浏览器朗读。Pages 工作流会取消同一提交自动触发的整仓部署，再由 GitHub Actions 创建一个不会触发分支构建的空标记提交，并显式发布约 3 MB 的网页与题库文件；它不重复打包音频，以避免大量小文件触发 GitHub Pages 的 10 分钟部署上限。本地运行仍保留全部音频源文件。
