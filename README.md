...
面试提升页位于 `#interview`，会从每天更新的 `jobs.json` 动态读取当前 A+ 与 A- 岗位。页面保留 12 道简历核心题和 16 道高频岗位题：核心题围绕新商户激活、权益 A/B、RFM 分层、产品体验闭环、销售赋能、行业支付方案和跨团队推进连续深挖；高频题只保留能由现有简历迁移证明的经营分析、客户需求、项目交付、合作谈判和行为面试。每题都有事实边界、回答框架、追问和失分点，题库可按关键词和方向筛选。页面聚焦选题、回答、反馈和范文，回答提示、真实经历卡与录音按需折叠；首次检查只给出一个应保留点和一个优先改进点，修改后可保存复练版、比较两个版本并完成一道追问。最多七张“真实经历卡”可复用到不同题目；核心范文只重组简历中可核验事实，选择具体岗位时会提醒按 JD 改写。
