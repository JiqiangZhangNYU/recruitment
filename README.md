...
面试提升页位于 `#interview`，会从每天更新的 `jobs.json` 动态读取当前 A+ 与 A- 岗位，并内置 WorldTrade 支付增长运营专项。页面保留 12 道核心训练题和 19 道高频岗位题，覆盖运营机制、经营复盘、预算与留存、商家供给、客户与伙伴、跨团队交付、海外市场和常见行为面试；题库可按关键词和方向筛选。页面聚焦选题、回答、反馈和范文，回答提示、真实经历卡与录音按需折叠；首次检查只给出一个应保留点和一个优先改进点，修改后可保存复练版、比较两个版本并完成一道追问。最多七张“真实经历卡”可复用到不同题目；固定 WorldTrade 范文会直接显示，选择其他岗位时仍保留改写提醒。
