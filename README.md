...
面试提升页位于 `#interview`，会从每天更新的 `jobs.json` 动态读取当前 A+ 与 A- 岗位及其能力维度，并内置 WorldTrade 支付增长运营专项。页面保留 12 道核心训练题，并增加 19 道高频岗位题，覆盖运营机制、经营复盘、预算与留存、商家供给、客户与伙伴、跨团队交付、海外市场和常见行为面试。“今日一题”按真实面试顺序安排练习；题库仍可按关键词、方向和岗位相关性筛选。每题先让用户完成首版回答，再按需展开框架、事实清单、常见追问和失分点；首次检查只给出一个应保留点和一个优先改进点，修改后可保存复练版、比较两个版本并完成一道追问。最多七张“真实经历卡”可复用到不同题目，固定 WorldTrade 范文在选择其他岗位时会隐藏不适用内容。
