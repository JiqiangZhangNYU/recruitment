...
网页包含独立的能力提升工作台。它以 68 个 A 档岗位为样本，首页介绍 13 项能力及其关系，并可从首页或侧栏进入单项训练；另有 16 周执行路线、14 项作品集和 A-/A+ 投递检查清单。业务英语已提供 6 个等级、30 道真实工作情境题：答案默认隐藏，确认掌握后逐题解锁，并支持关卡地图、题单、上一题和下一题导航。

通用学习内容独立存放在 `learning-guide.json`，业务英语题库存放在 `challenges/business-english.json`，并且只在进入业务英语时懒加载。所有学习与闯关进度只保存在当前浏览器。内容依据上级项目中的 `sources/recruit/A档岗位能力提升完整指南.md` 整理，因此刷新岗位数据时不会覆盖学习计划。
